...
import com.viewfunction.docg.analysisProvider.feature.ignite.memoryTable.query.SortingItem;
import com.viewfunction.docg.analysisProvider.feature.ignite.memoryTable.query.filteringItem.*;
import org.jooq.*;
...
public class MemoryTableQueryBuilder {

    private static Logger logger = LoggerFactory.getLogger(MemoryTableQueryBuilder.class);

    public static String buildSelectQuerySQL(String memoryTableName, QueryParameters queryParameters) throws MemoryTableQueryStructureException {
        ...
        if(queryParameters != null){
            ...
            if (defaultFilteringItem == null) {
                if ((andFilteringItemList != null && andFilteringItemList.size() > 0) ||
                    ...
                    MemoryTableQueryStructureException e = new MemoryTableQueryStructureException();
                ...
                if(queryParameters.isDistinctMode()){
                    selectConditionStep = create.selectDistinct(field("*")).from(table(memoryTableName)).where(defaultCondition);
                }else{
                    selectConditionStep = create.select(field("*")).from(table(memoryTableName)).where(defaultCondition);
                }
            ...
            if (startPage != 0) {
                if (startPage < 0) {
                    ...
                    MemoryTableQueryStructureException memoryTableQueryStructureException = new MemoryTableQueryStructureException();
                    memoryTableQueryStructureException.setCauseMessage(exceptionMessage);
                    throw memoryTableQueryStructureException;
                }
                if (pageSize < 0) {
            ...
            query = create.select(field("*")).from(table(memoryTableName));
        }
