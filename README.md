...
在 DOCG 数据分析平台中，表述领域模型的 概念实体（*ConceptionEntity*）与 关系实体（*RelationEntity*）技术上是存储在图数据库中的顶点（*vertex*）与边（*edge*），因此可以使用各种类型的图算法来针对领域模型执行高阶知识图谱分析。图算法主要
...
接近中心性：score=(numbers of nodes -1)/ sum(distance from node(u) to all other nodes)

调和中心性：score=sum (1/ distance from node(u) to all nodes)/ (numbers of nodes -1)



特征向量中心性算法：一种度量节点传递影响的算法。来自高分节点的关系比来自低分节点的连接对节点得分的贡献更大。高特征向量得分意味着一个节点连接到许多本身得分较高的节点。在计算过程中对于没有传入关系的节点，中心性分数将为0。 





PageRank算法：根据输入关系的数量和相应源节点的重要性来测量图中每个节点的重要性。
...
1. Louvain算法：Louvain算法是一种大型网络中检测社区的算法，评估一个社区内节点的连接密度。它以层次聚类的方式将集群递归地合并到一个节点上，同一集群的节点之间的关系成为自关系，改集群节点与其他集群的节点的关系连接到集群的代表点上。然后使用这个压缩图运行下一级别的集群运算。重复该过程，直到集群稳定。（下图为使用Louvain算法计算的例子，不考虑图中边的方向）（层次聚类方法还有Leiden算法，它是对Louvain算法的优化）
...
三角计数算法：该算法计算图中每个节点的三角形数量。三角形是由三个节点组成的集合，每个节点都与其他两个节点有关系。三角计数法在社会网络分析中越来越受欢迎，它被用来检测和衡量社区的凝聚力。也可以用来确定图的稳定性，并且经常被用作网络指数(如聚类系数)计算的一部分。还可用于计算局部聚类系数。（在此三角计数算法不考虑链接方向）



局部聚类系数算法：局部聚类系数描述任意节点的邻居节点也连通的可能性。计算方法为：

局部聚类系数=2*节点三角形个数/节点度数（节点度数-1）节点度数不考虑方向





在 DOCG 数据分析平台的核心领域模型 CoreRealm SDK 中 提供了 DataScienceOperator 工具， 它通过提供 可分析图（*AnalyzableGraph*） 功能的形式提供了近 40 种 常用 图算法的支持。
...
<img src="Documentation/intro/documentPic/banner.png" />