...
window.Vaadin.Flow['_vaadintheme_docg-pac_globalCss'] = window.Vaadin.Flow['_vaadintheme_docg-pac_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin.Flow['_vaadintheme_docg-pac_globalCss'].length === 0) || (!window.Vaadin.Flow['_vaadintheme_docg-pac_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin.Flow['_vaadintheme_docg-pac_globalCss'].push(target);
  }
